...
class RemoverDoCarrinho(View):
    def get(self, *args, **kwargs):
        http_refrer = self.request.META.get(
            'HTTP_REFERER',
            reverse('produto:lista')
        )
        variacao_id = self.request.GET.get('vid')

        if not variacao_id or not self.request.session.get('carrinho') or variacao_id not in self.request.session['carrinho']:
            return redirect(http_refrer)
